...
| `DEGENBOT_SOLVE_ADMISSION` | `solve.admission_shed` | `bool` | `false` | QTZGFL capacity-modulated admission stance (request-draw experiment): `1`/`true`/`on` replaces the in-flight cap degrade with budget = max(0, admission_target_depth - in-flight) and SHEDS zero-budget cycles; unset/`0`/`false` keeps the current degrade byte-identical (default OFF for the experiment). |
| `DEGENBOT_SOLVE_ADMISSION_TARGET_DEPTH` | `solve.admission_target_depth` | `usize` | `8` | QTZGFL: un-merged-result pipe depth target in KEYS (pools) — the draw budget headroom. Clamped at engine construction to 1..=DETACHED_INFLIGHT_CAP (the design-locked safety valve); default 8 = DETACHED_INFLIGHT_CAP. |
| `DEGENBOT_SOLVE_ADMISSION_RETENTION` | `solve.admission_retention_blocks` | `u64` | `50` | QTZGFL: retained (carried) key retention window W in blocks — on each block advance the ledger prunes buckets older than head - W and counts degenbot.detached.leads_expired_total. Default 50. |
