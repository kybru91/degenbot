...
/// VERIFY2 T1 (2026-09-05): the lagging-hop stage is SILENT on the console.
/// A pool trailing the anchor because it had no recent swap is NOT a fault:
/// (a) idle pools are expected (many pools swap than once per hour, let
/// alone every 10 blocks), (b) the sim's storage reads take the RPC
/// pre-state as the authority between checks, and (c) the overnight scan
/// measured 29k WARNs (1.3/s, unbounded) for exactly this non-fault. The
/// stage still computes the worst laggard for Stage 5 (the operator-gated
/// delivery-lag trip, default off) - and the previous REMED1 memo
/// (per-pool horizon) is superseded by growing none of it at WARN.
    ...
    if !lags_by_pool.is_empty() {
        ...
        tracing::debug!(
            ...
            "[solver-state] Tracked Live CL hop pools trail the solve anchor \
             (settle-lag baseline - no action; VERIFY2 on-demand instead)"
        );
    }
    worst
}
