...
mod tests {
    ...
    /// select’s await points; when the multi-threaded runtime migrated the
    /// task between workers, it entered on one thread and dropped on another,
    /// so the span stayed entered in the abandoned worker’s TLS — never
    /// closed, never exported, every child orphaned). The DETERMINISTIC
    /// defense is the structural fix (no `enter` guard may outlive a poll);
    /// this test locks the observable symptom — all N spans closed — and
    /// exercises cross-await parking so CI load that DOES migrate the task
    /// surfaces the old leak.
    ///
